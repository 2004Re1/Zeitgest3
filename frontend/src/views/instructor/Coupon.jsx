...
function Coupon() {
    ...
    const handleCouponSubmit = (e) => {
        ...
        useAxios.post(`teacher/coupon-list/${UserData()?.teacher_id}/`, formdata).then((res) => {
            console.log(res.data);
            fetchCoupons();
    ...
    const handleDeleteCoupon = (couponId) => {
        useAxios.delete(`teacher/coupon-detail/${UserData()?.teacher_id}/${couponId}/`).then((res) => {
            console.log(res.data);
            fetchCoupons();
            Toast().fire({
                icon: "success",
    ...
    const handleCouponUpdateSubmit = (e) => {
        ...
        useAxios.patch(`teacher/coupon-detail/${UserData()?.teacher_id}/${selectedCoupon.id}/`, formdata).then((res) => {
            console.log(res.data);
            fetchCoupons();
            handleClose();
            Toast().fire({
                                                        ...
                                                        <div className="mt-2">
                                                            <p className="mt-2">
                                                                <span className="me-2 fw-bold">
                                                                    Discount: <span className="fw-light">{c.discount}% Discount</span>
                                                                </span>
                                                            </p>
                                                            <p className="mt-1">
                                                                <span className="me-2 fw-bold">
                                                            ...
                                                            <p>
                                                                <button class="btn btn-outline-secondary" type="button" onClick={() => handleShow(c)}>
                                                                    Update Coupon
                                                                </button>

                                                                <button class="btn btn-danger ms-2" type="button" onClick={() => handleDeleteCoupon(c.id)}>
                ...
                <Modal.Body>
                    <form onSubmit={handleCouponUpdateSubmit}>
                        <div class="mb-3">
                            ...
                            <input type="text" placeholder="Code" defaultValue={selectedCoupon.code} className="form-control" name="code" onChange={handleCreateCouponChange} />
                            <label for="exampleInputEmail1" class="form-label mt-3">
                                Discount
                            </label>
                            <input type="text" placeholder="Discount" defaultValue={selectedCoupon.discount} className="form-control" name="discount" onChange={handleCreateCouponChange} id="" />
                        </div>
                ...
                <Modal.Body>
                    <form onSubmit={handleCouponSubmit}>
                        <div class="mb-3">
                            ...
                            <input type="text" placeholder="Code" value={createCoupon.code} className="form-control" name="code" onChange={handleCreateCouponChange} />
                            <label for="exampleInputEmail1" class="form-label mt-3">
                                Discount
                            </label>
                            <input type="text" placeholder="Discount" value={createCoupon.discount} className="form-control" name="discount" onChange={handleCreateCouponChange} id="" />
                        </div>
                        ...
                        <Button className="ms-2" variant="secondary" onClick={handleAddCouponClose}>
                            Close
                        </Button>
                    </form>
                </Modal.Body>
